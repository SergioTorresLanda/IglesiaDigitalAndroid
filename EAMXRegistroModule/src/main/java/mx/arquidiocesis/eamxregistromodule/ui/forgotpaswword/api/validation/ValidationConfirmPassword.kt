...
class ValidationConfirmPassword<T> : ValidationCode<Response<T>> {
    val gson = Gson()
    ...
    override fun executeValidation(response: Response<T>) {
        when (response.code()) {
            500 -> {
                val itemError =
                        //gson.fromJson(response.errorBody()?.source().toString().replace("[text=", "").replace("]", ""),
                        gson.fromJson(response.errorBody()?.source().toString().substring(response.errorBody()?.source().toString().indexOf("{"), response.errorBody()?.source().toString().indexOf(",")) + "}", ResponseError::class.java)
                when (itemError.code) {
                    108 -> {
                        throw BackendException("Has excedido el número máximo de intentos para cambiar la contraseña, espera unos minutos para intentarlo de nuevo.")
                    }
                    105 -> {
