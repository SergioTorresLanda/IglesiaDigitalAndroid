...
//        callBack.showToolbar(true, AppMyConstants.noticias)
        ...
        Glide.with(this@initElements).load(modelPublication.author.image).into(imgUser)
        txtNameChurch.buildTextSuccess(modelPublication.content)
        ...
        adapter.onItemClickListener = { media,index ->
            ...
            if (!(media.format.contains("image") || media.format.contains("jpeg") || media.format.contains("jpg"))) {
        ...
        layoutPlayer.setOnClickListener {
            ...
            EamxBottomSheetDialogFragment(idReaction, eamxBackHandler).show((activity as FragmentActivity).supportFragmentManager, EamxBottomSheetDialogFragment.TAG)
        }
    }
